...
test('tenant isolation rejects College B and preserves SUPER_ADMIN exception', async () => { const data = await fixture(); const collegeSession = data.auth.authenticate((await data.auth.login({ loginIdentifier: 'admin@a.test', password: data.password })).token); assert.throws(() => authorize(collegeSession, { tenantId: data.collegeB }), { code: 'FORBIDDEN' }); assert.doesNotThrow(() => authorize(collegeSession, { tenantId: data.collegeA })); const owner = data.auth.authenticate((await data.auth.login({ loginIdentifier: 'owner@test', password: data.password })).token); assert.doesNotThrow(() => authorize(owner, { tenantId: data.collegeB })); assert.equal(owner.user.college, null); data.db.close(); });
