...
export default function DailyClassSummary() {
  ...
  return <HodShell activeKey="class-teacher" title="Daily summary" subtitle={assignedClass ? `${assignedClass.batchName} · ${classTeacherDate()}` : classTeacherDate()} back backFallback="/hod/class-teacher">
    <PageContainer width="full" contentContainerStyle={{ maxWidth: sizing.contentMax }}>
      ...
      {!resource.loading && !resource.error && !assignedClass ? <StateView state={access.isClassTeacher ? 'empty' : 'unauthorized'} title={access.isClassTeacher ? 'No daily summary' : 'Class Teacher access required'} message={access.isClassTeacher ? 'The assigned class is not available in the current class list.' : 'Daily summaries are available only when the HOD is assigned as Class Teacher.'} /> : null}
