...
export default function ClassTeacherDashboard() {
  ...
  return <HodShell activeKey="class-teacher" title="Class teacher" subtitle={classTeacherDate()}>
    <PageContainer width="full" contentContainerStyle={{ maxWidth: sizing.contentMax }}>
      ...
      {!resource.loading && !resource.error && !assignedClass ? <StateView state={access.isClassTeacher ? 'empty' : 'unauthorized'} title={access.isClassTeacher ? 'Assignment unavailable' : 'Class Teacher access required'} message={access.isClassTeacher ? 'Your Class Teacher assignment is not available in the current class list.' : 'This workspace is available only when the HOD is assigned as Class Teacher.'} /> : null}
      {!resource.loading && !resource.error && assignedClass ? <>
        ...
        <Button label="View daily summary" onPress={() => router.push('/hod/class-teacher/daily-summary' as Href)} />
      </> : null}
    </PageContainer>
  </HodShell>;
}
