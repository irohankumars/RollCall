...
export default function NotificationPreferences() {
  ...
  return <HodShell activeKey="settings" title="Notification Preferences" subtitle="Choose useful updates" back backFallback="/hod/settings">
    <PageContainer width="compact">
      {state === 'loading' || restoring ? <LecturerPageSkeleton rows={6} /> : null}
      {state === 'error' ? <StateView state="error" message="Notification preferences are temporarily unavailable." onRetry={() => router.replace('/hod/settings/notifications' as Href)} /> : null}
