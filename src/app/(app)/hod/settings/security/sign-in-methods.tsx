...
export default function SignInMethods() {
  ...
  return <HodShell activeKey="settings" title="Sign-in Methods" subtitle="Linked ways to access RollCall" back backFallback="/hod/settings/security">
    <PageContainer width="compact">
      {state === 'loading' ? <LecturerPageSkeleton rows={4} /> : null}
      {state === 'error' ? <StateView state="error" message="Sign-in methods are temporarily unavailable." onRetry={() => router.replace('/hod/settings/security/sign-in-methods' as Href)} /> : null}
      ...
      {!state ? <>
        ...
        <View style={{ gap: spacing.xs }}>
          ...
          <Text style={[typography.body, { color: colors.textSecondary }]}>Each method below opens the same HOD account. Connecting a provider does not create a second RollCall account.</Text>
        </View>
