...
export default function AccountSecurity() {
  ...
  return <HodShell activeKey="settings" title="Account & Security" subtitle="Identity and sessions" back backFallback="/hod/settings">
    <PageContainer width="compact">
      {state === 'loading' ? <LecturerPageSkeleton rows={5} /> : null}
      {state === 'error' ? <StateView state="error" message="Security information is temporarily unavailable." onRetry={() => router.replace('/hod/settings/security' as Href)} /> : null}
      ...
      {!state ? <>
        ...
        <View style={{ gap: spacing.sm }}><SectionHeading title="Security" /><SettingsRow icon="key-outline" title="Sign-in Methods" detail="Manage methods linked to this RollCall identity" onPress={() => router.push('/hod/settings/security/sign-in-methods' as Href)} /><SettingsRow icon="help-buoy-outline" title="Account Recovery" detail="Learn how institution-managed recovery works" onPress={() => router.push('/hod/settings/security/recovery' as Href)} /><SettingsRow icon="shield-checkmark-outline" title="Security information" detail="Review account security guidance" onPress={() => router.push('/hod/settings/security/information' as Href)} /><SettingsRow icon="lock-closed-outline" title="Privacy" detail="Review privacy and data handling information" onPress={() => router.push('/hod/settings/about/privacy' as Href)} last /></View>
