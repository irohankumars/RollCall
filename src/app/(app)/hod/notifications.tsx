...
import { useHodNotifications, type NotificationCategory } from '@/hod/notification-provider';
...
export default function Notifications() {
  ...
  const { notifications, unreadCount: unread, markRead, markAllRead } = useHodNotifications();
  ...
  return <HodShell activeKey="notifications" title="Notifications" subtitle={`${unread} unread`}>
    <PageContainer width="standard">
      {state === 'loading' ? <LecturerPageSkeleton rows={5} /> : null}
      {state === 'error' ? <StateView state="error" message="Notifications could not be loaded." onRetry={() => router.replace('/hod/notifications' as Href)} /> : null}
      {state === 'empty' ? <View style={{ gap: spacing.md }}><StateView state="empty" title="No notifications" message="You&apos;re all caught up." /><Button label="Return to Today" variant="secondary" onPress={() => router.replace('/hod' as Href)} /></View> : null}
