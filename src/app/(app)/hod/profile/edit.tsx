...
import { useHodProfile, type HodProfileDraft } from '@/hod/profile-provider';
...
export default function EditLecturerProfile() {
  const { profile, loading, saveProfile } = useHodProfile();
  if (loading) return <HodShell activeKey="profile" title="Edit profile" subtitle="Personal information" back><PageContainer width="detail"><LecturerPageSkeleton rows={5} /></PageContainer></HodShell>;
  return <LoadedEditLecturerProfile profile={profile} saveProfile={saveProfile} />;
}

function LoadedEditLecturerProfile({ profile, saveProfile }: { profile: HodProfileDraft; saveProfile: (profile: HodProfileDraft) => Promise<void> }) {
  ...
  const leaveProfile = () => router.replace('/hod/profile' as Href);
  ...
  const submit = async () => {
    ...
    setSaving(false);
    router.replace('/hod/profile?saved=1' as Href);
  };
  ...
  return <HodShell activeKey="profile" title="Edit profile" subtitle="Personal information" back onBack={attemptLeave}>
