...
import { ClassSearch, LecturerClassRow, LecturerPageSkeleton, HodShell, ResourceState } from '@/hod/components';
...
export default function Classes() {
  ...
  const departmentAssignments = management.assignments.filter((item) => item.active && item.lecturerId === 'me').filter((assignment) => {
    const subject = management.subjects.find((item) => item.id === assignment.subjectId); const section = management.sections.find((item) => item.id === assignment.sectionId);
    return !resource.data?.some((item) => item.subjectCode === subject?.code && item.batchName === section?.name);
  });

  return <HodShell activeKey="classes" title="Classes" subtitle="Assigned subjects only">
    <PageContainer width="full" contentContainerStyle={{ maxWidth: sizing.contentMax }}>
      ...
      {departmentAssignments.length && filter === 'all' && !query.trim() ? <RelationshipSection title="New department assignments">{departmentAssignments.map((assignment) => { const subject = management.subjects.find((item) => item.id === assignment.subjectId); const section = management.sections.find((item) => item.id === assignment.sectionId); return <ManagementRow key={assignment.id} title={subject?.name ?? 'Subject'} detail={`${subject?.code} · ${section?.name}`} meta="Assigned to HOD / Me · Teaching schedule not yet configured" status="Assigned" onPress={() => router.push(`/hod/management/classes/${section?.id}` as Href)} />; })}</RelationshipSection> : null}
    </PageContainer>
  </HodShell>;
}
