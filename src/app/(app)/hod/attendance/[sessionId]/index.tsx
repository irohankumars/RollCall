...
function LocalSessionDetail({ classId, present, confirmedAt, method }: { classId: string; present?: string; confirmedAt?: string; method: AttendanceMethod }) {
  ...
  return item ? <SessionDetailContent item={item} lecturerName={session?.user.name ?? 'HOD'} /> : <PageContainer width="standard"><StateView state="error" title="Session not found" message="This local attendance session is unavailable." /></PageContainer>;
}
...
function SavedSessionDetail({ sessionId }: { sessionId: string }) {
  ...
  return resource.data ? <SessionDetailContent item={resource.data} lecturerName={session?.user.name ?? 'HOD'} /> : <PageContainer width="standard"><StateView state="error" title="Session not found" message="The requested attendance session could not be found." /></PageContainer>;
}
...
export default function SessionDetail() {
  ...
  return <HodShell activeKey="history" title="Session detail" back backFallback="/hod/history">
