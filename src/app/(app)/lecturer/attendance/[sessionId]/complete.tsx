...
export default function AttendanceComplete() {
  ...
  const actions = <View style={{ width: '100%', maxWidth: 620, alignSelf: 'center', flexDirection: isCompact ? 'column' : 'row', gap: spacing.sm }}>
    <Button label="View attendance" accessibilityHint="Opens the read-only attendance record" onPress={() => router.replace(`/lecturer/attendance/${sessionId}?${localQuery}` as Href)} style={{ flex: isCompact ? undefined : 1 }} />
    <Button label="Done" variant="secondary" accessibilityHint="Opens attendance history" onPress={() => router.replace(`/lecturer/history?${historyQuery}` as Href)} style={{ flex: isCompact ? undefined : 1 }} />
  </View>;

  return <LecturerShell activeKey="history" title="Attendance complete" subtitle={item?.subjectCode}>
