...
export default function ReviewAttendance() {
  ...
  const confirmLocal = () => {
    ...
    setConfirm(false);
    router.replace(`/lecturer/attendance/${sessionId}/complete?${query}` as Href);
  };

  return <LecturerShell activeKey="classes" title="Review attendance" subtitle={item?.subjectCode} back backFallback={`/lecturer/attendance/${sessionId}/face?classId=${encodeURIComponent(classId)}&method=${method}` as Href}>
