...
export default function StartAttendance() {
  ...
  const start = () => {
    ...
    router.replace(`/lecturer/attendance/${sessionId}/face?classId=${encodeURIComponent(item.id)}&method=${method}` as Href);
  };

  const chooseMethod = (next: AttendanceMethod) => { setMethod(next); if (next === 'MANUAL') setCameraState('required'); };

  return <LecturerShell activeKey="classes" title="Start attendance" subtitle={item?.subjectCode} back backFallback={`/lecturer/classes/${classId}` as Href}>
    <PageContainer width="compact" fixedActions={!loading && item && !alreadyActive ? <Button label="Start attendance" disabled={!canStart} accessibilityHint={canStart ? 'Opens the local attendance session' : 'Complete session readiness before starting'} onPress={start} /> : undefined}>
      ...
      {!loading && item && !alreadyActive ? <>
        ...
        <View style={{ gap: spacing.xs }}><Text style={[typography.caption, { color: colors.textMuted }]}>Lecturer</Text><Text style={[typography.body, { color: colors.textPrimary }]}>{session?.user.name ?? 'Lecturer'}</Text></View>
