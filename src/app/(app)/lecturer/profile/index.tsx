...
import { DetailRow, ProfileIdentity, RoleSummary, TeachingRow } from '@/lecturer/profile-components';
import { useLecturerProfile } from '@/lecturer/profile-provider';
...
export default function LecturerProfile() {
  ...
  const { profile, loading: profileLoading } = useLecturerProfile();
  ...
  return <LecturerShell activeKey="profile" title="Profile" subtitle="Lecturer account">
    <PageContainer width="detail">
      {loading ? <LecturerPageSkeleton rows={4} /> : null}
      {!loading && state === 'error' ? <StateView state="error" message="Your profile could not be loaded." onRetry={() => router.replace('/lecturer/profile' as Href)} /> : null}
      ...
      {!loading && !state && !classes.error ? <>
        ...
        <ProfileIdentity name={profile.displayName} designation="Lecturer" department="Computer Science and Engineering" onEdit={() => router.push('/lecturer/profile/edit' as Href)} />
