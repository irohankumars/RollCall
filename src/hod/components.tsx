...
import type { LecturerClass, SessionSummary } from '@/lecturer/types';
import { useHodUtilities } from './utility-provider';
...
function HeaderUtilities({ name }: { name: string }) {
  ...
  const { openQuickActions, openAccountMenu } = useHodUtilities();
  return <View style={{ flexDirection: 'row', alignItems: 'center' }}>
    <IconButton label="Open global search" onPress={() => router.push('/hod/search' as Href)}><Ionicons name="search-outline" size={sizing.iconMd} color={colors.textSecondary} /></IconButton>
...
export function HodShell({ activeKey, title, subtitle, back, backFallback = '/hod', onBack, children, trailing }: { activeKey: string; title: string; subtitle?: string; back?: boolean; backFallback?: Href | string; onBack?: () => void; children: React.ReactNode; trailing?: React.ReactNode }) {
  const { session } = useAuth(); const navigation = useNavigation(); const name = session?.user.name ?? 'HOD';
  const { unreadCount } = useHodNotifications(); const access = hodAccessFor(session?.user);
  const handleBack = () => {
    if (onBack) { onBack(); return; }
    if (navigation.canGoBack()) { navigation.goBack(); return; }
    router.replace(backFallback as Href);
  };
  return <AppShell navigation={hodNavigation(access.isClassTeacher, unreadCount)} activeKey={activeKey} role="hod" header={<CompactHeader title={title} subtitle={subtitle} backAction={back ? handleBack : undefined} trailing={trailing ?? <HeaderUtilities name={name} />} />}>{session?.user.role === 'HOD' ? children : <StateView state="unauthorized" />}</AppShell>;
}
...
export function TodayClassRow({ item }: { item: LecturerClass }) {
  ...
  return <Pressable accessibilityRole="button" accessibilityLabel={`${formatClassTime(item.nextSessionAt)}, ${item.subjectName}, ${item.batchName}, ${status}`} accessibilityHint="Opens class details" onPress={() => router.push(`/hod/classes/${item.id}` as Href)} style={({ pressed }) => ({ minHeight: 76, paddingVertical: spacing.md, flexDirection: 'row', alignItems: 'center', gap: spacing.lg, borderBottomWidth: 1, borderBottomColor: colors.borderSubtle, opacity: pressed ? 0.68 : 1 })}><View style={{ width: 68 }}><Text style={[typography.numeric, { color: colors.textPrimary }]}>{formatClassTime(item.nextSessionAt)}</Text></View><View style={{ flex: 1, gap: spacing.xxs }}><Text style={[typography.subheading, { color: colors.textPrimary }]}>{item.subjectName}</Text><Text style={[typography.bodySmall, { color: colors.textSecondary }]}>{item.subjectCode} · {item.batchName}</Text></View><Badge label={status} tone={current ? 'info' : 'neutral'} /><Ionicons accessibilityElementsHidden name="chevron-forward" size={sizing.iconSm} color={colors.textMuted} /></Pressable>;
}
...
export function LecturerPageSkeleton({ rows = 3 }: { rows?: number }) {
  return <View accessibilityLabel="Loading HOD information" style={{ gap: spacing.xl }}><View style={{ gap: spacing.sm }}><Skeleton width="34%" height={14} /><Skeleton width="62%" height={28} /><Skeleton width="48%" height={16} /></View><View style={{ padding: spacing.xl, gap: spacing.md }}><Skeleton width="30%" height={14} /><Skeleton width="74%" height={26} /><Skeleton width="52%" /></View><View style={{ gap: spacing.lg }}>{Array.from({ length: rows }, (_, index) => <View key={index} style={{ gap: spacing.sm, paddingVertical: spacing.md }}><Skeleton width="66%" height={20} /><Skeleton width="44%" /><Skeleton width="82%" height={12} /></View>)}</View></View>;
}
